...
public interface EventRepository extends JpaSpecificationRepository<Event> {
	...
	@Query(value = "select ec.id, ec.name, count(e.id) as total from mvr_event e inner join mvr_event_cases ec ON e.ecase_id =ec.id where e.opstina_id=?1 && e.datum>=DATE_FORMAT(?2, '%Y-%c-%e %H:%i:%s') && e.datum <=DATE_FORMAT(?3, '%Y-%c-%e %H:%i:%s') group by ec.id ", nativeQuery = true)
